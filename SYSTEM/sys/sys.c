...
/**
 * 1.SCB_DisableICache函数，用于关闭ICache。
 * 2.SCB_DisableDCache函数，用于关闭DCache。
 * 3.SCB_InvalidateDCache函数，用于丢弃DCache当前数据，重新从SRAM获取数据。
 * 4.SCB_CleanDCache函数，用于将DCache数据回写到SRAM里面，同步数据。
 * 5.SCB_CleanInvalidateDCache函数，用于回写数据到SRAM，并重新获取DCache数据。
 * 开启了DCache的透写模式，这样带来的好处就是可以保证DCache和SRAM里面数据的一致性，
 * 坏处就是会损失一定的性能（每次都要回写数据）
 */
//使能CPU的L1-Cache
void Cache_Enable(void)
{
    // I/D Cache可以缓存指令/数据，提高CPU访问指令/数据的速度，从而大大提高MCU的性能
    SCB_EnableICache();//使能I-Cache
    SCB_EnableDCache();//使能D-Cache
    // 控制D Cache数据的回写，以获得最佳性能，则可以关闭D Cache透写模式
